...
class AsyncSecureSysClient(BaseAsyncClient):
    ...
    async def register_or_get_system(self, hostname: str, os: str, environment: str = "production") -> Dict[str, Any]:
        ...
        try:
            response = await self._request_with_retry(
                "GET", f"{self.api_url}/api/v1/scanning/systems/by_hostname/", params={"hostname": hostname}
            )
    ...
    async def submit_scan(
        self, system_id: str, scan_payload: Dict[str, Any], scan_type: ScanType = ScanType.FULL
    ) -> Dict[str, Any]:
        """
    ...
    async def submit_scans_batch(self, scans: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        tasks = [
            self.submit_scan(scan["system_id"], scan["scan_payload"], scan.get("scan_type", ScanType.FULL))
            for scan in scans
        ]
    ...
    async def get_scan_status(self, scan_id: str) -> Dict[str, Any]:
        ...
        response = await self._request_with_retry("GET", f"{self.api_url}/api/v1/scanning/scans/{scan_id}/")
    ...
    async def get_findings(
        self, scan_id: Optional[str] = None, severity: Optional[str] = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """
        ...
        response = await self._request_with_retry("GET", f"{self.api_url}/api/v1/scanning/findings/", params=params)
...
async def run_scan_async(
    api_url: str, api_key: str, scan_type: ScanType = ScanType.FULL, minimal_telemetry: bool = True, **client_kwargs
) -> Dict[str, Any]:
    """
    ...
    async with AsyncSecureSysClient(api_url, api_key, **client_kwargs) as client:
        ...
        # Submit scan
        scan_result = await client.submit_scan(system_id=system["id"], scan_payload=scan_data, scan_type=scan_type)

        # Wait for completion
        final_result = await client.wait_for_scan_completion(scan_id=scan_result["scan_id"], timeout=120.0)
