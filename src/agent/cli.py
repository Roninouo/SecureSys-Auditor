...
async def _submit_scan_async(config, scan_result, scan_type, wait, verbose):
    ...
    async with AsyncSecureSysClient(
        api_url=config.api_url, api_key=config.api_key, allow_insecure_localhost=config.allow_insecure_localhost
    ) as client:
        # Health check
        ...
        # Submit scan
        result = await client.submit_scan(system_id=system["id"], scan_payload=scan_result, scan_type=scan_type)

        click.echo(click.style("✓ Scan submitted successfully!", fg="green"))
        ...
        if wait:
            ...
            try:
                final_result = await client.wait_for_scan_completion(scan_id=result["scan_id"], timeout=120.0)
...
@cli.command()
@click.option("--output", "-o", type=click.Choice(["json", "yaml", "text"]), default="json", help="Output format")
...
@click.option(
    "--scan-type",
    type=click.Choice(["full", "quick", "compliance", "vulnerability"]),
    default="full",
...
def scan(output, submit, file, verbose, use_async, scan_type, wait):
    ...
    if submit:
        ...
        if use_async:
            # Use async client
            result = asyncio.run(_submit_scan_async(config, scan_result, scan_type_enum, wait, verbose))
        else:
            # Use sync client
