...
import pytest
from webhooks.models import DeadLetterQueue, WebhookDelivery, WebhookEndpoint
...
class TestWebhookIdempotency:
    def test_idempotency_key_uniqueness(self):
        ...
        endpoint = WebhookEndpoint.objects.create(name="Test Endpoint", url="http://example.com/webhook")

        key = "unique-key-123"

        # First delivery
        WebhookDelivery.objects.create(
            endpoint=endpoint, event_type="scan.completed", payload={"scan_id": 1}, idempotency_key=key
        )
...
class TestDeadLetterQueue:
    def test_move_to_dlq(self):
        ...
        delivery = WebhookDelivery.objects.create(
            endpoint=endpoint, event_type="scan.failed", status=WebhookDelivery.Status.FAILED, attempt_count=5
        )
    ...
    def test_dlq_reprocessing(self, mock_deliver):
        ...
        delivery = WebhookDelivery.objects.create(
            endpoint=endpoint, event_type="scan.failed", status=WebhookDelivery.Status.DLQ
        )

        dlq_entry = DeadLetterQueue.objects.create(
            original_delivery=delivery, reason=DeadLetterQueue.Reason.MAX_RETRIES
        )
