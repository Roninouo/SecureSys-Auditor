...
class WebhookService:
    ...
    def deliver(self, endpoint: WebhookEndpoint, event_type: str, payload: Dict[str, Any]) -> DeliveryResult:
        """
        ...
        if endpoint.is_circuit_open():
            ...
            return DeliveryResult(
                endpoint_id=str(endpoint.id), endpoint_url=endpoint.url, success=False, error="Circuit breaker open"
            )
        ...
        try:
            response = self._session.post(endpoint.url, json=webhook_payload, headers=headers, timeout=30)
    ...
    def deliver_to_all(self, event_type: str, payload: Dict[str, Any]) -> List[DeliveryResult]:
        """
        ...
        for endpoint in endpoints:
            ...
            WebhookDelivery.objects.create(
                ...
                event_type=event_type,
                payload=payload,
                status=(WebhookDelivery.Status.SUCCESS if result.success else WebhookDelivery.Status.FAILED),
    ...
    def _sign_payload(self, payload: Dict[str, Any], secret: Optional[str]) -> str:
        """
        ...
        if not secret_key:
            raise ValueError("Webhook signing requires an endpoint secret or WEBHOOK_SECRET to be set")
