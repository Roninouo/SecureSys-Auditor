...
from .models import AuditLog, Finding, Recommendation, Scan, SecurityMaturityAssessment, System
...
class UserCreateSerializer(serializers.ModelSerializer):
    ...
    password = serializers.CharField(write_only=True, min_length=8)  # pragma: allowlist secret
...
class SystemListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = System
        fields = ["id", "hostname", "os", "environment", "last_seen", "latest_risk_score", "latest_maturity_level"]
...
class ScanSubmitSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        scan = Scan.objects.create(
            system=system, scan_payload=validated_data["scan_payload"], status=Scan.Status.PENDING
        )
        return scan
...
class WebhookEndpointSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {"secret": {"write_only": True}}  # Don't expose secret in responses
...
class ReportGenerationSerializer(serializers.Serializer):
    ...
    report_type = serializers.ChoiceField(choices=["executive", "technical", "compliance"], default="executive")
    ...
    def validate_scan_id(self, value):
        ...
        if not Scan.objects.filter(id=value, status="completed").exists():
            raise serializers.ValidationError("Scan not found or not completed.")
        return value
