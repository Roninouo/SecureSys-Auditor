...
class RateLimitMiddleware:
    ...
    def __call__(self, request):
        ...
        allowed, retry_after = self.check_rate_limit(
            limit_key, self.limits.get(user_type, 20), self.burst.get(user_type, 5)
        )

        if not allowed:
            logger.warning(
                f"Rate limit exceeded for {limit_key}",
                extra={"user_type": user_type, "path": request.path, "method": request.method},
            )
    ...
    def check_rate_limit(self, key: str, rate: int, burst: int) -> tuple[bool, Optional[int]]:
        """
        ...
        # Convert rate/minute to rate/second
        ...
        if tokens >= 1.0:
            ...
            tokens -= 1.0
            # 2 minute expiry
            cache.set(key, (tokens, now), timeout=120)
            return True, None
        else:
                ...
                return JsonResponse(
                    {
                        "error": "rate_limit_exceeded",
                        "message": (
                            f"Rate limit exceeded for this endpoint. Limit: {self.rate} requests per {self.period} "
                            "seconds."
                        ),
                    },
                    status=429,
