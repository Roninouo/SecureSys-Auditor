...
class EnhancedSecurityAnalyzer(SecurityAnalyzer):
    ...
    def analyze_with_maturity(self) -> Dict[str, Any]:
        ...
        roadmap = self._generate_improvement_roadmap(
            domain_scores, nist_scores, base_result['findings']
        )
    ...
    def _calculate_domain_scores(self, findings: List[Dict]) -> Dict[str, int]:
        ...
        domain_scores = {domain: 4 for domain in self.DOMAIN_WEIGHTS.keys()}
        ...
        return {domain: max(1, min(4, round(score))) 
    ...
    def _calculate_nist_scores(self, findings: List[Dict]) -> Dict[str, int]:
        ...
        return {func: max(1, min(4, round(score))) 
    ...
    def _generate_improvement_roadmap(
        ...
        if critical_findings:
            roadmap.append({
                ...
                'domains_improved': list(set(
                    self.CATEGORY_TO_DOMAIN.get(f.get('category', 'other'), 'other')
        ...
        for domain, score in low_domains[:3]:  # Top 3 lowest domains
            ...
            roadmap.append({
                ...
                'effort': 'Medium',
                'impact': f'Domain maturity improvement',
        ...
        low_nist = [
            (func, score) for func, score in nist_scores.items()
            if score < 3
        ]
