...
def process_scan(self, scan_id: str):
    ...
    """
    from .models import Finding, Recommendation, Scan

    try:
        scan = Scan.objects.get(id=scan_id)
        scan.mark_processing()

        logger.info(f"Processing scan {scan_id}", extra={"scan_id": scan_id, "system_id": str(scan.system.id)})
...
def cleanup_old_scans(days: int = 90):
    ...
    old_scans = Scan.objects.filter(scan_date__lt=cutoff_date, status=Scan.Status.COMPLETED)
    ...
    logger.info(f"Cleaned up {count} old scans", extra={"scans_cleaned": count, "cutoff_date": str(cutoff_date)})
...
def generate_daily_report():
    ...
    critical_unresolved = Finding.objects.filter(severity="critical", is_resolved=False).count()

    high_unresolved = Finding.objects.filter(severity="high", is_resolved=False).count()
...
def generate_pdf_report(
    self, scan_id: str, report_type: str = "executive", company_name: str = "Organization", notify_webhook: bool = False
):
    """
    ...
    from .reports import WEASYPRINT_AVAILABLE, generate_scan_report
    ...
    try:
        ...
        # Generate PDF
        pdf_bytes = generate_scan_report(scan_id=scan_id, report_type=report_type, company_name=company_name)
        ...
        logger.info("PDF report generated successfully", extra=result)

        # Send webhook notification if enabled
        if notify_webhook:
            send_webhook_notification.delay(event_type="report.generated", payload=result)
    ...
    try:
        ...
        for endpoint in endpoints:
            try:
                ...
                # Send webhook
                response = requests.post(endpoint.url, json=webhook_payload, headers=headers, timeout=30)
                ...
                logger.info(
                    f"Webhook sent to {endpoint.url}",
                    extra={"event_type": event_type, "status_code": response.status_code},
                )

            except requests.RequestException as e:
                logger.error(
                    f"Webhook delivery failed: {endpoint.url}", extra={"error": str(e), "event_type": event_type}
                )
                results.append(
                    {
                        "endpoint": endpoint.url,
