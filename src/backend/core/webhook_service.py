...
class WebhookCircuitBreaker:
    ...
    def __init__(
        self, endpoint_id: str, failure_threshold: int = 5, recovery_timeout: int = 60, success_threshold: int = 2
    ):
        """
    ...
    def record_failure(self):
        ...
        if state == CircuitState.HALF_OPEN:
            ...
            cache.set(self.cache_key_opened_at, time.time(), timeout=3600)
            ...
            if failures >= self.failure_threshold:
                self.set_state(CircuitState.OPEN)
                cache.set(self.cache_key_opened_at, time.time(), timeout=3600)
                logger.error(f"Circuit opened for endpoint {self.endpoint_id} " f"after {failures} failures")
...
class WebhookRateLimiter:
    ...
    def can_attempt(self) -> bool:
        ...
        if current >= self.max_requests:
            logger.warning(f"Rate limit exceeded for endpoint {self.endpoint_id}: " f"{current}/{self.max_requests}")
            return False
...
class EnhancedWebhookService:
    ...
    def send_webhook(
        ...
        if not rate_limiter.can_attempt():
            return {"success": False, "error": "rate_limited", "message": "Endpoint rate limit exceeded"}
        ...
        if not circuit_breaker.can_attempt():
            logger.warning(f"Circuit breaker open for endpoint {endpoint_id}, " "skipping delivery")
            return {"success": False, "error": "circuit_breaker_open", "message": "Endpoint circuit breaker is open"}
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                response = session.post(endpoint_url, json=webhook_payload, headers=headers, timeout=timeout)
                ...
                if success:
                    ...
                    logger.warning(
                        ...
                        extra={"endpoint_url": endpoint_url, "status_code": response.status_code, "error": last_error},
                    )
                ...
                logger.error(
                    ...
                    extra={"endpoint_url": endpoint_url, "error": str(e), "attempt": attempt},
                )
        ...
        return {"success": False, "error": "max_retries_exceeded", "message": last_error, "attempts": max_retries}

    def send_to_all_endpoints(self, event_type: str, payload: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
