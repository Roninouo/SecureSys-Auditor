...
class Command(BaseCommand):
    ...
    def create_backup(self):
        ...
        with transaction.atomic():
            ...
            backup_data = {"timestamp": timestamp, "tables": existing_tables, "data": {}}
            ...
            try:
                ...
                if "core_system" in existing_tables:
                    backup_data["data"]["systems"] = list(core_models.System.objects.values())
                if "core_scan" in existing_tables:
                    backup_data["data"]["scans"] = list(core_models.Scan.objects.values())
                if "core_finding" in existing_tables:
                    backup_data["data"]["findings"] = list(core_models.Finding.objects.values())
                if "core_recommendation" in existing_tables:
                    backup_data["data"]["recommendations"] = list(core_models.Recommendation.objects.values())
