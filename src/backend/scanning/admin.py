...
class SystemAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'hostname', 'os', 'environment', 'is_active',
        'latest_risk_score', 'latest_maturity_level', 'last_seen'
    ]
    ...
    fieldsets = (
        (None, {
            'fields': ('hostname', 'os', 'os_version', 'environment')
        }),
        ('Network', {
            'fields': ('ip_address', 'description')
        }),
        ('Status', {
            'fields': ('is_active', 'latest_risk_score', 'latest_maturity_level')
        }),
        ('Timestamps', {
...
class ScanAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'id_short', 'system_hostname', 'scan_type', 'status',
        'risk_score_display', 'maturity_level', 'scan_date'
    ]
    list_filter = ['status', 'maturity_level', 'scan_type']
    search_fields = ['id', 'system__hostname']
    readonly_fields = [
        'id', 'created_at', 'updated_at', 'scan_date',
    ...
    fieldsets = (
        (None, {
            'fields': ('system', 'scan_type', 'status')
        }),
        ('Results', {
            'fields': ('risk_score', 'maturity_level', 'score_breakdown')
        }),
        ('Data', {
            'fields': ('scan_payload', 'error_message'),
            'classes': ('collapse',)
        }),
        ('Timestamps', {
...
class FindingAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'title', 'category', 'severity_display', 'scan_hostname',
    ...
    def severity_display(self, obj):
        ...
        return format_html(
            ...
            colors.get(obj.severity, 'gray'), obj.severity
        )
