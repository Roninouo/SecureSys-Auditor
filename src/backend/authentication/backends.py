...
from .providers import AuthProvider, AuthResult, KeycloakOIDCProvider, SimpleJWTProvider
...
class ProviderChainAuthentication(authentication.BaseAuthentication):
    ...
    def _init_providers(self):
        ...
        provider_classes = getattr(
            settings,
            'AUTH_PROVIDERS',
            [KeycloakOIDCProvider, SimpleJWTProvider]
        )
        ...
        logger.debug(
            f"Initialized auth providers: {[p.name for p in self._providers]}"
        )
