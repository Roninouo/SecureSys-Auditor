...
weasyprint_skip = pytest.mark.skipif(
    ...
    reason="WeasyPrint not available (requires GTK libraries)"
)
...
class TestReportService:
    ...
    def test_service_rejects_pending_scan(self, db):
        ...
        from scanning.models import System, Scan
        from reports.services import ReportService
        ...
        scan = Scan.objects.create(
            system=system,
            scan_payload={},
            status=Scan.Status.PENDING  # Not completed
        )
...
class TestReportEndpoint:
    ...
    def test_synchronous_report_generation(self, processed_scan, rf):
        ...
        # Create request
        request = rf.post('/reports/generate/', {
            'scan_id': str(processed_scan.id),
            'report_type': 'executive',
            'async': False,
