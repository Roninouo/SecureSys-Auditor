...
from core.webhook_service import CircuitState, WebhookCircuitBreaker, WebhookRateLimiter, webhook_service
...
class MockWebhookHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        self.requests_received.append(
            {"path": self.path, "headers": dict(self.headers), "body": json.loads(body) if body else None}
        )
...
class TestWebhookCircuitBreaker:
    ...
    def test_circuit_half_open_after_timeout(self):
        ...
        cb = WebhookCircuitBreaker("test-endpoint-3", failure_threshold=2, recovery_timeout=1)  # 1 second
    ...
    def test_circuit_closes_after_successes(self):
        ...
        cb = WebhookCircuitBreaker("test-endpoint-4", failure_threshold=2, recovery_timeout=1, success_threshold=2)
...
class TestWebhookRateLimiter:
    ...
    def test_rate_limiter_allows_within_limit(self):
        ...
        limiter = WebhookRateLimiter("test-endpoint-5", max_requests=5, window=10)
    ...
    def test_rate_limiter_blocks_over_limit(self):
        ...
        limiter = WebhookRateLimiter("test-endpoint-6", max_requests=3, window=10)
...
class TestWebhookService:
    ...
    def test_send_webhook_retry_on_failure(self, mock_webhook_server):
        ...
        test_requests = [
            r for r in MockWebhookHandler.requests_received if r["body"] and r["body"].get("event_type") == "test.event"
        ]
    ...
    def test_send_to_all_endpoints(self, mock_filter, mock_webhook_server):
        ...
        results = webhook_service.send_to_all_endpoints(event_type="test.event", payload={"message": "test"})
