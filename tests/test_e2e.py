...
class TestE2EWorkflow:
    ...
    def test_complete_scan_workflow(
        ...
        assert response.status_code in [200, 201, 202], \
            f"Scan submission failed: {response.text}"
            ...
            if response.status_code == 200:
                ...
                if scan_status["status"] in ["completed", "failed"]:
                    ...
                    assert scan_status["status"] == "completed", \
                        f"Scan failed: {scan_status.get('error_message')}"
                    break
        ...
        telnet_finding = next(
            (f for f in findings if "Telnet" in f.get("title", "")),
            None
        )
        ...
        if system_id:
            response = requests.get(
                f"{base_url}/api/systems/{system_id}/",
                headers=user_auth_headers,
                timeout=10
            )
    ...
    def test_list_scans_pagination(self, base_url, user_auth_headers):
        ...
        response = requests.get(
            f"{base_url}/api/scans/?page=1&page_size=10",
            headers=user_auth_headers,
            timeout=10
        )
    ...
    def test_agent_authentication_required(self, base_url, sample_scan_data):
        ...
        assert response.status_code in [401, 403], \
            "Unauthenticated scan submission should be rejected"
    ...
    def test_invalid_scan_data_rejected(self, base_url, agent_auth_headers):
        ...
        assert response.status_code == 400, \
            "Invalid scan data should be rejected with 400"
    ...
    def test_webhook_delivery(self, base_url, user_auth_headers):
        ...
        response = requests.get(
            f"{base_url}/api/webhooks/deliveries/?event_type=scan.completed",
            headers=user_auth_headers,
            timeout=10
        )
    ...
    def test_recommendations_generated(
        ...
        response = requests.get(
            f"{base_url}/api/scans/{scan_id}/recommendations/",
            headers=user_auth_headers,
            timeout=10
        )
...
class TestE2EPerformance:
    ...
    def test_api_response_time(self, base_url, user_auth_headers):
        ...
        for endpoint in endpoints:
            ...
            assert elapsed < 2.0, \
                f"Endpoint {endpoint} took {elapsed:.2f}s (should be < 2s)"
...
class TestE2ESmoke:
    ...
    def test_celery_workers(self):
        ...
        response = requests.get(
            "http://localhost:8000/health/celery/",
            timeout=10
        )
        assert response.status_code == 200
