...
class TestProcessScanTask:
    ...
    def scan_with_payload(self, system, sample_scan_payload):
        ...
        return Scan.objects.create(
            system=system,
            scan_payload=sample_scan_payload
        )
    ...
    def secure_scan(self, system, secure_scan_payload):
        ...
        return Scan.objects.create(
            system=system,
            scan_payload=secure_scan_payload
        )
    ...
    def old_scans(self):
        ...
        for i in range(3):
            scan = Scan.objects.create(
                system=system,
                scan_payload={'data': f'test-{i}'},
                status=Scan.Status.COMPLETED
            )
            # Manually set old date
            Scan.objects.filter(id=scan.id).update(
                scan_date=timezone.now() - timedelta(days=100)
            )
            scans.append(scan)
